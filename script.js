jQuery(document).ready(function($) {
	...
	$(document).on("submit", "#messenger", function(e) {
		...
		if( val.indexOf("hello") > -1 || val.indexOf("hi") > -1 || val.indexOf("good morning") > -1 || val.indexOf("good afternoon") > -1 || val.indexOf("good evening") > -1 || val.indexOf("good night") > -1 ) {
		...
		} else if( val.indexOf("check symptoms") > -1) {
			if (val.indexOf("symptoms") > -1 || val.indexOf("gf") > -1) {

				appendMsg("check the GET CONSULTATION section on the home page");
				appendMsg("is it helpful? (yes/no)");

			} else if (val.indexOf("doctors") > -1 || val.indexOf("bf") > -1) {
			...
			if(val.indexOf("my name is") > -1) {
				name = val.replace("my name is Aagam", "");
			}

			else if(val.indexOf("i am") > -1) {
				name = val.replace("i am aagam", "");
			}

			else if(val.indexOf("i'm") > -1) {
				name = val.replace("i'm aagam", "");
			}
			...
			appendMsg("Hi " + name + ", we are Aagam, how can we help you?");
		} else {
